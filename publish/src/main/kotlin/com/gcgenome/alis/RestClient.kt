...
class RestClient(private val client: RestTemplate) {
    fun fileUpload(fileUpload: FileUpload) : Boolean{
        val requestEntity = HttpEntity(fileUpload, MultiValueMapAdapter(mapOf("Content-Type" to listOf("application/json"))))
    ...
    fun state(request: Request, state: String, member: String, machine: String): Mono<Boolean> {
        val requestEntity = HttpEntity(request, MultiValueMapAdapter(mapOf("Content-Type" to listOf("application/json"))))
