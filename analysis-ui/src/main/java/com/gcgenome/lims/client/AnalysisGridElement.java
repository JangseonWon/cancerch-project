...
//		if(!Objects.equals(data.get("Comment"), ""))	iptComment.value(data.get("Comment"));
	...
	private Data map(Analysis value) {
		...
		String comment		= value.comment().equals("") || value.comment().equals(" ") ? "입력" : "수정";
	...
	public Analysis[] selection() {
		return Arrays.stream(wrapper.selection()).map(d->d.get("ID").replace("-", "") + "$" + d.get("검사코드") + "$" + d.get("Batch") + "$" + d.get("Row")).map(values::get).toArray(Analysis[]::new);
	}
	@Override
