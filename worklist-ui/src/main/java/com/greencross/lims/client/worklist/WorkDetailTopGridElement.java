...
public class WorkDetailTopGridElement extends HTMLElementBuilder<HTMLDivElement, WorkDetailTopGridElement> implements HasSelectionChangeHandlers<Stream<Request>> {
    public static WorkDetailTopGridElement instance() { return new WorkDetailTopGridElement(div()); }
    private enum COLUMN_KEY {
        NO, RECEIPT, ID, TYPE, CUSTOMER_NAME, MRN, PATIENT_NAME, PATIENT_CODE, SEX, SPECIMEN_TYPE, END_DT, REMARK, INFO
    }
    ...
    private void layout() {
        ...
        config.columns(
                ColumnBuilder.string(COLUMN_KEY.NO.name()).name("No").width(60).readOnly(true).horizontal("center").build(),
                ColumnBuilder.string(COLUMN_KEY.RECEIPT.name()).name("접수일").width(100).readOnly(true).build(),
                ColumnBuilder.string(COLUMN_KEY.ID.name()).name("검체번호").width(100).readOnly(true).build(),
                ColumnBuilder.string(COLUMN_KEY.TYPE.name()).name("검사명").width(100).readOnly(true).build(),
                ColumnBuilder.string(COLUMN_KEY.CUSTOMER_NAME.name()).name("의뢰기관").width(100).readOnly(true).build(),
                ColumnBuilder.string(COLUMN_KEY.MRN.name()).name("MRN").width(100).readOnly(true).build(),
                ColumnBuilder.string(COLUMN_KEY.PATIENT_NAME.name()).name("수진자명").width(100).readOnly(true).build(),
                ColumnBuilder.string(COLUMN_KEY.PATIENT_CODE.name()).name("수진자코드").width(100).readOnly(true).build(),
                ColumnBuilder.string(COLUMN_KEY.SEX.name()).name("성별").width(100).readOnly(true).horizontal("center").build(),
                ColumnBuilder.string(COLUMN_KEY.SPECIMEN_TYPE.name()).name("검체종류").width(100).readOnly(true).horizontal("center").build(),
                ColumnBuilder.string(COLUMN_KEY.END_DT.name()).name("완료예정일").width(100).readOnly(true).horizontal("right").build(),
                ColumnBuilder.string(COLUMN_KEY.REMARK.name()).name("REMARK").width(100).readOnly(true).build(),
                ColumnBuilder.string(COLUMN_KEY.INFO.name()).name("검체정보").width(100).readOnly(true).build()
        ).stretchH("all");
    }
    ...
    private static Data map(Request value) {
        ...
        return new Data(String.valueOf(value.sample()))
                .put(COLUMN_KEY.NO.name(), String.valueOf(++No))
    ...
    @Override
    public HandlerRegistration onSelectionChange(SelectionChangeEventListener<Stream<Request>> listener) {
        return onSelectionChange(sheet.element(), listener);
    }

    @Override
    public HandlerRegistration onSelectionChange(EventTarget dom, SelectionChangeEventListener<Stream<Request>> listener) {
