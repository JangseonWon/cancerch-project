...
class SectionFemsScore(private val y: Float = 487f): Painter<DNACXTemplate<DNACXResource>, DNACXDto> {
    override fun paint(
        ...
        stream.setLineWidth(2f).setStrokingColor(Color(186,26,50)).setNonStrokingColor(Color(186,26,50))
            .setLineDashPattern(floatArrayOf(1.7f, 2f), 1f).moveTo(135f+(dto.result.femsScoreCutOff.toFloat()*100f*4.1f), y - BACKGROUND_RATE - 30f).lineTo(135f+(dto.result.femsScoreCutOff.toFloat()*100f*4.1f), y - BACKGROUND_RATE + 62f).stroke()
        stream.restoreGraphicsState()
        stream.saveGraphicsState()
        stream.setLineWidth(1.5f).setStrokingColor(Color(67,72,143)).setNonStrokingColor(Color(67,72,143))
            .setLineDashPattern(floatArrayOf(2f, 1.7f), 3f).moveTo(135f+(dto.result.femsScore.toFloat()*100)*4.1f, y - BACKGROUND_RATE - 30f).lineTo(135f+(dto.result.femsScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 62f).stroke()
        stream.circle(135f+(dto.result.femsScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 62f, 2.5f).fill()
        stream.restoreGraphicsState()
        stream.saveGraphicsState()
        stream.paragraph(135f+(dto.result.femsScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 90f, 200f, AlignHorizontal.CENTER, AlignVertical.MIDDLE, TextBlock(template.resource().styleContentBold().clone().color(Color(0,0,0)).fontSize(10f), "あなたのスコア"))
        stream.setLineWidth(0.8f).setStrokingColor(Color(0,0,0)).setNonStrokingColor(Color(0,0,0)).moveTo(135f+(dto.result.femsScore.toFloat()*100)*4.1f-35, y - BACKGROUND_RATE + 82f).lineTo(135f+(dto.result.femsScore.toFloat()*100)*4.1f+37f,y - BACKGROUND_RATE + 82f).stroke()
