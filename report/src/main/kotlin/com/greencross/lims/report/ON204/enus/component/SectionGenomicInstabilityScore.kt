...
class SectionGenomicInstabilityScore(private val y: Float = 295f): Painter<DNACXTemplate<DNACXResource>, DNACXDto> {
    override fun paint(
        ...
        stream.paragraph(42f, y - SMALL_TITLE_RATE-5, 200f, AlignHorizontal.LEFT, TextBlock(template.resource().styleContentBold().clone().fontSize(13f).color(Color(255,255,255)), " Genomic Instability Score"))
        stream.paragraph(205f, y- SMALL_TITLE_RATE-5, 400f, AlignHorizontal.LEFT, TextBlock(template.resource().styleContentRegualar().clone().fontSize(12f).color(Color(114,113,113)), "The genomic instability score represents the overall extent of\ngenome-wide copy number alterations."))
        ...
        stream.setLineWidth(2f).setStrokingColor(Color(186,26,50)).setNonStrokingColor(Color(186,26,50))
            .setLineDashPattern(floatArrayOf(1.7f, 2f), 1f).moveTo(137f+((dto.result.genomicInstabilityCutOff.toFloat()-3)*100*0.5857f), y - BACKGROUND_RATE - 56f).lineTo(137f+((dto.result.genomicInstabilityCutOff.toFloat()-3)*100*0.5857f), y - BACKGROUND_RATE + 42f).stroke()
        stream.restoreGraphicsState()
        stream.saveGraphicsState()
        stream.setLineWidth(1.5f).setStrokingColor(Color(67,72,143)).setNonStrokingColor(Color(67,72,143))
            .setLineDashPattern(floatArrayOf(2f, 1.7f), 3f).moveTo(137f+((dto.result.genomicInstability.toFloat()-3)*100*0.5857f), y - BACKGROUND_RATE - 56f).lineTo(137f+((dto.result.genomicInstability.toFloat()-3)*100*0.5857f),y - BACKGROUND_RATE + 42f).stroke()
        stream.circle(137f+((dto.result.genomicInstability.toFloat()-3)*100*0.5857f),y - BACKGROUND_RATE + 42f, 2.5f).fill()
        stream.restoreGraphicsState()
        stream.saveGraphicsState()
        stream.paragraph(137f+((dto.result.genomicInstability.toFloat()-3)*100*0.5857f),y - BACKGROUND_RATE + 70f, 200f, AlignHorizontal.CENTER, AlignVertical.MIDDLE, TextBlock(template.resource().styleContentBold().clone().color(Color(0,0,0)).fontSize(10f), "Your Result"))
        stream.setLineWidth(0.8f).setStrokingColor(Color(0,0,0)).setNonStrokingColor(Color(0,0,0)).moveTo(137f+((dto.result.genomicInstability.toFloat()-3)*100*0.5857f)-30, y - BACKGROUND_RATE + 62f).lineTo(137f+((dto.result.genomicInstability.toFloat()-3)*100*0.5857f)+32f,y - BACKGROUND_RATE + 62f).stroke()
