...
class SectionTitle(private val y: Float = 745f): Painter<DNACXTemplate<DNACXResource>, DNACXDto> {
    override fun paint(
        ...
        stream.paragraph(firstHeaderLine, y + 40f,   50f,  AlignHorizontal.LEFT, TextBlock(template.resource().styleContentBold().fontSize(9f), "依頼機関"))
        stream.paragraph(firstHeaderLine, y + 24f,   100f,  AlignHorizontal.LEFT, TextBlock(template.resource().styleContentBold().fontSize(9f), "検体"))
        stream.paragraph(secondHeaderLine,y + 24f,   100f, AlignHorizontal.LEFT, TextBlock(template.resource().styleContentBold().fontSize(9f), "患者ID"))
        stream.paragraph(firstHeaderLine, y + 8f,    50f,  AlignHorizontal.LEFT, TextBlock(template.resource().styleContentBold().fontSize(9f), "検体番号"))
        stream.paragraph(secondHeaderLine,y + 8f,    100f, AlignHorizontal.LEFT, TextBlock(template.resource().styleContentBold().fontSize(9f), "検体採取日"))
        stream.paragraph(firstHeaderLine, y - 6f,    50f,  AlignHorizontal.LEFT, TextBlock(template.resource().styleContentBold().fontSize(9f), "年齢/性別"))
        stream.paragraph(secondHeaderLine,y - 6f,    100f, AlignHorizontal.LEFT, TextBlock(template.resource().styleContentBold().fontSize(9f), "受付日/報告日"))
