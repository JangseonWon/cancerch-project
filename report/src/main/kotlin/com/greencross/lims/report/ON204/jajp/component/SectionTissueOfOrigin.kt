...
class SectionTissueOfOrigin(private val y: Float = 398f): Painter<DNACXTemplate<DNACXResource>, DNACXDto> {
    override fun paint(
        ...
        val (color, comment) =
            ...
            else Pair(Color(114,113,113), " 該当なし")

        stream.paragraph(35f, y-14f, 400f, AlignHorizontal.LEFT, TextBlock(template!!.resource().styleContentBold().clone().color(Color(67,72,142)).fontSize(15f), "がん種:"),
        ...
        if(!checker) {
            ...
            stream.paragraph(300f, y- CANCER_BOX_RATE - 80, 200f, AlignHorizontal.CENTER, TextBlock(template.resource().styleContentBold().clone().color(Color(114,113,113)).fontSize(13f), "該当なし"))
        }
    ...
    private fun drawCancerBox(stream: PDPageContentStreamPageAccessible, template: DNACXTemplate<DNACXResource>, cancer: String, x: Float, y: Float, checker: Boolean) {
        ...
        stream.paragraph(x+12, y - CANCER_BOX_RATE/2 - 15, 100f, AlignHorizontal.CENTER, TextBlock(template.resource().styleContentBold().clone().color(if(checker)Color(186,26,50) else Color(114,113,113)).fontSize(13f), if(checker) "シグナルあり" else "該当なし"))
