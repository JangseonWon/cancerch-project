...
class SectionCovScore(private val y: Float = 669f): Painter<DNACXTemplate<DNACXResource>, DNACXDto> {
    override fun paint(
        ...
        stream.paragraph(42f, y - SMALL_TITLE_RATE+3, 200f, AlignHorizontal.LEFT, TextBlock(template.resource().styleContentBold().clone().fontSize(13f).color(Color(255,255,255)), "COV Score"))
        stream.paragraph(120f, y- SMALL_TITLE_RATE+3, 400f, AlignHorizontal.LEFT, TextBlock(template.resource().styleContentRegualar().clone().fontSize(12f).color(Color(114,113,113)), "The COV score represents coverage patterns at the whole genome level."))
        ...
        stream.setLineWidth(2f).setStrokingColor(Color(186,26,50)).setNonStrokingColor(Color(186,26,50))
            .setLineDashPattern(floatArrayOf(1.7f, 2f), 1f).moveTo(135f+(dto.result.covScoreCutOff.toFloat()*100f*4.1f), y - BACKGROUND_RATE - 30f).lineTo(135f+(dto.result.covScoreCutOff.toFloat()*100f*4.1f), y - BACKGROUND_RATE + 68f).stroke()
        stream.restoreGraphicsState()
        stream.saveGraphicsState()
        stream.setLineWidth(1.5f).setStrokingColor(Color(67,72,143)).setNonStrokingColor(Color(67,72,143))
            .setLineDashPattern(floatArrayOf(2f, 1.7f), 3f).moveTo(135f+(dto.result.covScore.toFloat()*100)*4.1f, y - BACKGROUND_RATE -30f).lineTo(135f+(dto.result.covScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 68f).stroke()
        stream.circle(135f+(dto.result.covScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 68f, 2.5f).fill()
        stream.restoreGraphicsState()
        stream.saveGraphicsState()
        stream.paragraph(135f+(dto.result.covScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 96f, 200f, AlignHorizontal.CENTER, AlignVertical.MIDDLE, TextBlock(template.resource().styleContentBold().clone().color(Color(0,0,0)).fontSize(10f), "Your Result"))
        stream.setLineWidth(0.8f).setStrokingColor(Color(0,0,0)).setNonStrokingColor(Color(0,0,0)).moveTo(135f+(dto.result.covScore.toFloat()*100)*4.1f-30, y - BACKGROUND_RATE + 88f).lineTo(135f+(dto.result.covScore.toFloat()*100)*4.1f+32f,y - BACKGROUND_RATE + 88f).stroke()
