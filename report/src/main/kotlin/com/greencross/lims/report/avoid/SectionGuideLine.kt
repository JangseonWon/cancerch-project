        ...
        if(dto.result != AvoidDto.Results.GENERAL) {
            when(dto.result){
                AvoidDto.Results.RISK -> {
                    ...
                    stream.paragraph(380f, y + RESULT_CONTENT_RATE - 79 + CONTENT_CANCER_RATE * 0.5f, 160f, AlignHorizontal.CENTER,
                        TextBlock(styleBold.clone().color(Color(255, 255, 255)).fontSize(10f),  template.lblGuideLineTableHeader2()))
                    stream.paragraph(380f, y+RESULT_CONTENT_RATE-77, 160f, AlignHorizontal.CENTER,
                        TextBlock(styleRegular.clone().fontSize(10f), template.lblGuideLineTime(dto.first.name)))
                    stream.paragraph(153f, y+64, 400f, AlignHorizontal.LEFT,
                        TextBlock(styleRegular.clone().fontSize(8f), template.lblGuideLineComment(dto.first.name)))
                }
            }
            ...
            stream.paragraph(343f, y+ RESULT_CONTENT_LOW_RATE-207, 230f, AlignHorizontal.LEFT, TextBlock(styleRegular.clone().fontSize(7.5f), template.lblGuideLineNormalTest(5f)))
