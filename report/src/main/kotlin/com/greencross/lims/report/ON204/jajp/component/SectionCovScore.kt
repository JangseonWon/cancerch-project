...
class SectionCovScore(private val y: Float = 669f): Painter<DNACXTemplate<DNACXResource>, DNACXDto> {
    override fun paint(
        ...
        stream.setLineWidth(2f).setStrokingColor(Color(186,26,50)).setNonStrokingColor(Color(186,26,50))
            .setLineDashPattern(floatArrayOf(1.7f, 2f), 1f).moveTo(145f+(dto.result.covScoreCutOff.toFloat()*100f*4.1f), y - BACKGROUND_RATE - 26f).lineTo(145f+(dto.result.covScoreCutOff.toFloat()*100f*4.1f), y - BACKGROUND_RATE + 68f).stroke()
        stream.restoreGraphicsState()
        stream.saveGraphicsState()
        stream.setLineWidth(1.5f).setStrokingColor(Color(67,72,143)).setNonStrokingColor(Color(67,72,143))
            .setLineDashPattern(floatArrayOf(2f, 1.7f), 3f).moveTo(145f+(dto.result.covScore.toFloat()*100)*4.1f, y - BACKGROUND_RATE - 26f).lineTo(145f+(dto.result.covScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 68f).stroke()
        stream.circle(145f+(dto.result.covScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 68f, 2.5f).fill()
        stream.restoreGraphicsState()
        stream.saveGraphicsState()
        stream.paragraph(145f+(dto.result.covScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 96f, 200f, AlignHorizontal.CENTER, AlignVertical.MIDDLE, TextBlock(template.resource().styleContentBold().clone().color(Color(0,0,0)).fontSize(10f), "あなたのスコア"))
        stream.setLineWidth(0.8f).setStrokingColor(Color(0,0,0)).setNonStrokingColor(Color(0,0,0)).moveTo(145f+(dto.result.covScore.toFloat()*100)*4.1f-35, y - BACKGROUND_RATE + 88f).lineTo(145f+(dto.result.covScore.toFloat()*100)*4.1f+37f,y - BACKGROUND_RATE + 88f).stroke()
        stream.paragraph(145f+(dto.result.covScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 79f, 200f, AlignHorizontal.CENTER, AlignVertical.MIDDLE, TextBlock(template.resource().styleContentBold().clone().color(if(dto.result.covScore >= dto.result.covScoreCutOff) Color(186, 26, 50) else Color(111,186,44)).fontSize(14f), "${dto.result.covScore}"))
        stream.restoreGraphicsState()
        stream.saveGraphicsState()
