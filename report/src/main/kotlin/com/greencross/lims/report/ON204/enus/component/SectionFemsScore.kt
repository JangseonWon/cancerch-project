...
class SectionFemsScore(private val y: Float = 487f): Painter<DNACXTemplate<DNACXResource>, DNACXDto> {
    override fun paint(
        ...
        stream.paragraph(42f, y - SMALL_TITLE_RATE-5, 200f, AlignHorizontal.LEFT, TextBlock(template.resource().styleContentBold().clone().fontSize(13f).color(Color(255,255,255)), "FEMS Score"))
        stream.paragraph(120f, y- SMALL_TITLE_RATE-5, 400f, AlignHorizontal.LEFT, TextBlock(template.resource().styleContentRegualar().clone().fontSize(12f).color(Color(114,113,113)), "The FEMS score represents changes in cfDNA size and end motifs patterns."))
        ...
        stream.setLineWidth(2f).setStrokingColor(Color(186,26,50)).setNonStrokingColor(Color(186,26,50))
            .setLineDashPattern(floatArrayOf(1.7f, 2f), 1f).moveTo(135f+(dto.result.femsScoreCutOff.toFloat()*100f*4.1f), y - BACKGROUND_RATE - 36f).lineTo(135f+(dto.result.femsScoreCutOff.toFloat()*100f*4.1f), y - BACKGROUND_RATE + 62f).stroke()
        stream.restoreGraphicsState()
        stream.saveGraphicsState()
        stream.setLineWidth(1.5f).setStrokingColor(Color(67,72,143)).setNonStrokingColor(Color(67,72,143))
            .setLineDashPattern(floatArrayOf(2f, 1.7f), 3f).moveTo(135f+(dto.result.femsScore.toFloat()*100)*4.1f, y - BACKGROUND_RATE - 36f).lineTo(135f+(dto.result.femsScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 62f).stroke()
        stream.circle(135f+(dto.result.femsScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 62f, 2.5f).fill()
        stream.restoreGraphicsState()
        stream.saveGraphicsState()
        stream.paragraph(135f+(dto.result.femsScore.toFloat()*100)*4.1f,y - BACKGROUND_RATE + 90f, 200f, AlignHorizontal.CENTER, AlignVertical.MIDDLE, TextBlock(template.resource().styleContentBold().clone().color(Color(0,0,0)).fontSize(10f), "Your Result"))
        stream.setLineWidth(0.8f).setStrokingColor(Color(0,0,0)).setNonStrokingColor(Color(0,0,0)).moveTo(135f+(dto.result.femsScore.toFloat()*100)*4.1f-30, y - BACKGROUND_RATE + 82f).lineTo(135f+(dto.result.femsScore.toFloat()*100)*4.1f+32f,y - BACKGROUND_RATE + 82f).stroke()
